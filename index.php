          ...
          <form class="d-flex" action="crud_estudiante.php" method="post">
            ...
            <div class="mb-3">
              ...
              <div class="mb-3">
                ...
                <input type="text" name="txt_carnet" id="txt_carnet" class="form-control" placeholder="Codigo: A001" required>
              </div>
              <div class="mb-3">
                <label for="lbl_nombres" class="form-label"><b>Nombres</b></label>
                <input type="text" name="txt_nombres" id="txt_nombres" class="form-control" placeholder="Nombres: ESDRAS JOEL" required>
              </div>
              <div class="mb-3">
                <label for="lbl_apellidos" class="form-label"><b>Apellidos</b></label>
                <input type="text" name="txt_apellidos" id="txt_apellidos" class="form-control" placeholder="Apellidos: DIAZ GONZALEZ" required>
              </div>
              <div class="mb-3">
                <label for="lbl_direccion" class="form-label"><b>Direccion</b></label>
                <input type="text" name="txt_direccion" id="txt_direccion" class="form-control" placeholder="Direccion: LOTE 177 COL. ROSALINDA ESCUINTLA" required>
              </div>
              <div class="mb-3">
                <label for="lbl_telefono" class="form-label"><b>Telefono</b></label>
                <input type="number" name="txt_telefono" id="txt_telefono" class="form-control" placeholder="Telefono: 666" required>
              </div>
              <div class="mb-3">
                <label for="lbl_puesto" class="form-label"><b>Puesto</b></label>
                <select class="form-select" name="genero" id="MASCULINO">
                </div>
              <div class="mb-3">
                <label for="lbl_email" class="form-label"><b>email</b></label>
                <input type="text" name="txt_email" id="txt_email" class="form-control" placeholder="email: esdraskater_1986@hotmail.es" required>
              </div>
              <div class="mb-3">
                <label for="lbl_fecha_nacimiento" class="form-label"><b>fecha_nacimiento</b></label>
                <input type="text" name="txt_fecha_nacimiento" id="txt_fecha_nacimiento" class="form-control" placeholder="fn: 02/05/1986" required>
              </div>
              <div class="mb-3">
                <label for="lbl_estudiante" class="form-label"><b>estudiantes</b></label>
                <select class="form-select" name="drop_estudiante" id="drop_estudiante">
                  <option value=0> ---- Estudiante ---- </option>
                  <?php 
                   include("datos_conexion.php");
                   $db_conexion = mysqli_connect($db_host,$db_usr,$db_pass,$db_nombre);
                   $db_conexion -> real_query ("SELECT id_estudiante as estuadiante FROM estudiante;");
                  $resultado = $db_conexion -> use_result();
                  while ($fila = $resultado ->fetch_assoc()){
                    echo "<option value=". $fila['id'].">". $fila['estudiante']."</option>";
    ...
    <table class="table table-striped table-inverse table-responsive">
      <thead class="thead-inverse">
        ...
        <tbody id="tbl_estudiante">
         ...
         include("datos_conexion.php");
         $db_conexion = mysqli_connect($db_host,$db_usr,$db_pass,$db_nombre);
         $db_conexion -> real_query ("SELECT e.id_estudiante as id,e.carnet,e.nombres,e.apellidos,e.direccion,e.telefono,e.genero,e.fecha_nacimiento,p.id_estudiante FROM bd_escuela as e inner join estudiante as p on e.id_estudiante = p.id_estudiante;");
        $resultado = $db_conexion -> use_result();
        while ($fila = $resultado ->fetch_assoc()){
